...
public class PlayerHitchhikeManager : NetworkBehaviour
{
    ...
    NetworkVariable<ulong> activeHandAreaId = new NetworkVariable<ulong>(
        ulong.MaxValue,
        NetworkVariableReadPermission.Everyone,
        NetworkVariableWritePermission.Owner
    );
    ...
    public override void OnNetworkSpawn()
    {
        ...
        originalHandAreaId.OnValueChanged += (previousValue, newValue) =>
        {
            ...
            LocalHitchhikeManager.Instance.handAreaManager.handAreas.ForEach(area =>
        ...
        activeHandAreaId.OnValueChanged += (previousValue, newValue) =>
        {
            ...
            if (LocalHitchhikeManager.Instance.handAreaManager.handAreas == null) return;
    ...
    IEnumerator SeekActiveAreaLoop(ulong activeAreaId)
    {
        ...
        var newArea = LocalHitchhikeManager.Instance.handAreaManager.handAreas.Find(area => area.GetComponent<NetworkObject>().NetworkObjectId == activeAreaId);
        var i = LocalHitchhikeManager.Instance.handAreaManager.handAreas.FindIndex(area => area == newArea);
        if (i != -1)
        {
            foreach (var item in LocalHitchhikeManager.Instance.handAreaManager.handAreas.Select((area, index) => new { area, index }))
            {
            ...
            // dnd grab
            if (!LocalHitchhikeManager.Instance.DragAndDrop
                || interactables == null
            ...
            foreach (var (rawInteractable, handIndex) in interactables.Select((value, index) => (value, index)))
            {
                ...
                var newPose = HitchhikeUtilities.ApplyOffset(grabbable.transform.GetPose(Space.World), LocalHitchhikeManager.Instance.beforeCoordTransform, newCoord.transform, Vector3.zero);
                grabbable.transform.SetPose(newPose, Space.World);
                var newScale = LocalHitchhikeManager.Instance.scaleHandModel ? HitchhikeUtilities.ApplyScaling(LocalHitchhikeManager.Instance.beforeCoordTransform, newCoord.transform) : 1;
    ...
    void Update()
    {
        ...
        var handAreaManager = LocalHitchhikeManager.Instance.handAreaManager;
        ...
        // switching
        var newActiveHandAreaIndex = LocalHitchhikeManager.Instance.switchTechnique.GetFocusedHandAreaIndex(activeHandAreaIndex);
        if (activeHandAreaIndex != newActiveHandAreaIndex)
        {
            ...
            LocalHitchhikeManager.Instance.beforeCoordTransform.position = beforeCoordinate.transform.position;
            LocalHitchhikeManager.Instance.beforeCoordTransform.rotation = beforeCoordinate.transform.rotation;
            LocalHitchhikeManager.Instance.beforeCoordTransform.localScale = beforeCoordinate.transform.lossyScale;
